...
def Classification_Performance_Relation(collection, gold_standard_ids, gold_standard_relations, homolo = False, threshold = 0.5):
    correct = prediction_count = 0
    ...
    if homolo:
        gold_standard_relations_homo = set()
        homoloQuerier = HomoloQueryService()
        for relation_str in gold_standard_relations:
            pmid, g1, g2 = relation_str.split('_')
            pmid = pmid[4:]
            g1_h = homoloQuerier.getHomolo(g1)
            g2_h = homoloQuerier.getHomolo(g2)
            relation_string = 'PMID' + pmid + '_' + '_'.join(sorted([g1_h, g2_h]))
            gold_standard_relations_homo.add(relation_string)
        gold_standard_relations = gold_standard_relations_homo
    for document in collection.get('documents', []):
        id, label, confidence, relations = JSON_Document(document)
                ...
                if 'infons' in relation:
                    ...
                    for infon_type in infons:
                        ...
                        if infon_type_lowercase == 'relation':
                            ...
                            geneid = infons[infon_type]
                            if homolo:
                                geneid = homoloQuerier.getHomolo(geneid)
                            infon_values.append(geneid)
                        elif infon_type_lowercase == 'confidence':
                            relation_confidence = float(infons[infon_type])
                            # if relation_confidence < threshold:
                                # continue
